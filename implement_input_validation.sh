...
# Script to implement comprehensive input validation and sanitization

echo "Implementing comprehensive input validation and sanitization..."

# Create validation helper for each service
create_validation_helper() {
    local service_path="$1"
    local service_name="$2"

    # Create internal/validation directory if it doesn't exist
    mkdir -p "$service_path/internal/validation"

    # Create validation helper file
    cat > "$service_path/internal/validation/validator.go" << 'EOF'
package validation
...
EOF
    ...
    echo "Created validation helper for $service_name"
}

# Find all service directories (excluding shared-resilience)
find microservices -mindepth 1 -maxdepth 1 -type d -not -name "shared-resilience" | while read -r service_dir; do
    service_name=$(basename "$service_dir")
    echo "Processing service: $service_name"

    create_validation_helper "$service_dir" "$service_name"

    # Update service main.go files to include validation middleware
    main_file="$service_dir/cmd/main.go"
    if [ -f "$main_file" ]; then
        # Check if validation import exists
        if ! grep -q "validation.*internal/validation" "$main_file"; then
            # Add validation import (this is a simplified approach)
            echo "// TODO: Add validation import to $main_file manually"
        fi
    fi
done

echo "Input validation implementation complete!"
echo ""
echo "Manual steps required:"
echo "1. Update each service's main.go to import the validation package"
echo "2. Initialize validator in service setup"
echo "3. Add validation middleware to Gin router"
echo "4. Update handler functions to use validation helpers"
echo ""
echo "Example integration in main.go:"
echo 'import "your-service/internal/validation"'
echo 'validator := validation.NewServiceValidator(logger)'
echo 'router.Use(validator.MiddlewareFunc())'