// Package config provides configuration management for the Incident Service.
package config
...
// Config represents the Incident Service configuration.
type Config struct {
...
func Load() (*Config, error) {
	config := &Config{
		...
		Database: DatabaseConfig{
			...
			Name:     getEnv("DB_NAME", "statuspage_incidents"),
		...
		JWT: JWTConfig{
			...
			Issuer:     getEnv("JWT_ISSUER", "statuspage-incident-service"),
		},
		Service: ServiceConfig{
			Name:        getEnv("SERVICE_NAME", "incident-service"),
			Version:     getEnv("SERVICE_VERSION", "1.0.0"),
			Description: getEnv("SERVICE_DESCRIPTION", "Incident Management Service for Status Page"),
			Tags:        getEnvSlice("SERVICE_TAGS", []string{"incident-service", "microservice"}),
		},
		Monitoring: MonitoringConfig{
			Enabled:     getEnvBool("MONITORING_ENABLED", true),
			MetricsPort: getEnvInt("METRICS_PORT", 9096),
			HealthPort:  getEnvInt("HEALTH_PORT", 8087),
