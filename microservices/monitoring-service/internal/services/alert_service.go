// Package services provides alert management business logic.
package services
...
func (s *AlertService) GetAlerts(tenantID uint, status string, limit, offset int) ([]*models.Alert, int64, error) {
	var alerts []*models.Alert
	var total int64
...
func (s *AlertService) GetAlertStatistics(tenantID uint, startDate, endDate time.Time) (map[string]interface{}, error) {
	...
	query := s.db.Model(&models.Alert{}).Where("tenant_id = ?", tenantID)
	if !startDate.IsZero() {
