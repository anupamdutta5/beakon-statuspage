...
func Load() (*Config, error) {
	config := &Config{
		...
		Database: DatabaseConfig{
			...
			Name:     getEnv("DB_NAME", "statuspage_payments"),
		...
		JWT: JWTConfig{
			...
			Issuer:     getEnv("JWT_ISSUER", "statuspage-payment-service"),
		},
		Service: ServiceConfig{
			Name:        getEnv("SERVICE_NAME", "payment-service"),
			Version:     getEnv("SERVICE_VERSION", "1.0.0"),
			Description: getEnv("SERVICE_DESCRIPTION", "Payment Management Service for Status Page"),
			Tags:        getEnvSlice("SERVICE_TAGS", []string{"payment-service", "microservice"}),
		},
		Monitoring: MonitoringConfig{
			Enabled:     getEnvBool("MONITORING_ENABLED", true),
			MetricsPort: getEnvInt("METRICS_PORT", 9098),
		...
		Payment: PaymentConfig{
			Stripe: StripeConfig{
				...
				IsEnabled:      getEnvBool("STRIPE_ENABLED", false),
			},
			PayPal: PayPalConfig{
				ClientID:     getEnv("PAYPAL_CLIENT_ID", ""),
				ClientSecret: getEnv("PAYPAL_CLIENT_SECRET", ""),
				WebhookID:    getEnv("PAYPAL_WEBHOOK_ID", ""),
				IsEnabled:    getEnvBool("PAYPAL_ENABLED", false),
				IsSandbox:    getEnvBool("PAYPAL_SANDBOX", true),
			},
			Razorpay: RazorpayConfig{
				KeyID:         getEnv("RAZORPAY_KEY_ID", ""),
				KeySecret:     getEnv("RAZORPAY_KEY_SECRET", ""),
				WebhookSecret: getEnv("RAZORPAY_WEBHOOK_SECRET", ""),
				IsEnabled:     getEnvBool("RAZORPAY_ENABLED", false),
			},
			DefaultCurrency: getEnv("DEFAULT_CURRENCY", "USD"),
			WebhookSecret:   getEnv("WEBHOOK_SECRET", "development-webhook-secret"),
		},
	}
