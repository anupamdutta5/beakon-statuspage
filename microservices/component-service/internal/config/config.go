// Package config provides configuration management for the Component Service.
package config
...
// Config represents the Component Service configuration.
type Config struct {
...
func Load() (*Config, error) {
	config := &Config{
		...
		JWT: JWTConfig{
			...
			Issuer:     getEnv("JWT_ISSUER", "statuspage-component-service"),
		},
		Service: ServiceConfig{
			Name:        getEnv("SERVICE_NAME", "component-service"),
			Version:     getEnv("SERVICE_VERSION", "1.0.0"),
			Description: getEnv("SERVICE_DESCRIPTION", "Component Management Service for Status Page"),
			Tags:        getEnvSlice("SERVICE_TAGS", []string{"component-service", "microservice"}),
		},
		Monitoring: MonitoringConfig{
			Enabled:     getEnvBool("MONITORING_ENABLED", true),
			MetricsPort: getEnvInt("METRICS_PORT", 9094),
			HealthPort:  getEnvInt("HEALTH_PORT", 8085),
