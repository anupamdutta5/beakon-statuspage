// Package config provides configuration management for the Billing Consumer.
package config
...
// Config represents the Billing Consumer configuration.
type Config struct {
...
// BillingConfig represents billing-specific configuration.
type BillingConfig struct {
	ProcessingEnabled bool    `yaml:"processing_enabled"`
	InvoiceEnabled    bool    `yaml:"invoice_enabled"`
	PaymentEnabled    bool    `yaml:"payment_enabled"`
	TaxEnabled        bool    `yaml:"tax_enabled"`
...
func Load() (*Config, error) {
	config := &Config{
		...
		Service: ServiceConfig{
			Name:        getEnv("SERVICE_NAME", "billing-consumer"),
			Version:     getEnv("SERVICE_VERSION", "1.0.0"),
			Description: getEnv("SERVICE_DESCRIPTION", "Billing Consumer for Status Page"),
			Tags:        getEnvSlice("SERVICE_TAGS", []string{"billing-consumer", "microservice"}),
		},
		Queue: QueueConfig{
			...
			QueueName:   getEnv("QUEUE_NAME", "billing"),
		...
		Database: DatabaseConfig{
			...
			Name:     getEnv("DB_NAME", "statuspage_billing"),
		...
		Billing: BillingConfig{
			...
			InvoiceEnabled:    getEnvBool("INVOICE_ENABLED", true),
			PaymentEnabled:    getEnvBool("PAYMENT_ENABLED", true),
			TaxEnabled:        getEnvBool("TAX_ENABLED", true),
			...
			DeadLetterQueue:   getEnv("DEAD_LETTER_QUEUE", "billing-dlq"),
